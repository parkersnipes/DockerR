...
muni_pol <- readOGR("Data/col muni polygons/col_admbnda_adm2_mgn_20200416.shp")
...
calculateDF <- function(k,h,distanceMetric,granularity,epicenter,a,b,c,d,e){
  #print(width())
  #print(k())
  for(i in 1:1120){
    for(j in 1:1120){
      ...
      if(are.connected(munigraph,i,j)){
        ...
        if(i<nrow(cross_section_merged) & j<nrow(cross_section_merged)){
          ...
          temp <- d1 * exp(a*elev_difference) * exp(b * average_ruggedness) * exp(c * average_slope) * exp(d * average_forest) * exp(e * average_reflectiveness)
  ...
  Vtot_final <- mutate(Vtot_final, ring_num = as.integer(10*delta_min / max(Vtot_final$deltamin)))
...
ui <- fluidPage(
  ...
  sliderInput('a','Select parameter a (distance 2 parameter)',min = 1,max = 2,value = 1,step = 0.1),
  sliderInput('b','Select parameter b (distance 2 parameter)',min = 1,max = 2,value = 1,step = 0.1),
  sliderInput('c','Select parameter c (distance 2 parameter)',min = 1,max = 2,value = 1,step = 0.1),
  sliderInput('d','Select parameter d (distance 2 parameter)',min = 1,max = 2,value = 1,step = 0.1),
  sliderInput('e','Select parameter e (distance 2 parameter)',min = 1,max = 2,value = 1,step = 0.1),
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ...
    d <- reactive(input$d)
    e <- reactive(input$e)
    yearshare_df_1<- calculateDF(k(),h(),distanceMetric(),granularity(),epicenter(),a(),b(),c(),d(),e())
